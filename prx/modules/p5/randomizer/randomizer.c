...
void GetEncounterEntryFromTBLHook( int encounterID, encounterIDTBL* tbl, encounterIDTBL* (*originalFnc) (int))
{
  if (isRandomizerEnabled() && CONFIG_ENABLED(randomizeEncounters)) {
    ...
    if (tbl->flags.byte == randTbl->flags.byte/*CONFIG_ENABLED(randomizeEnemies)*/) tbl->flags.eventScript = 0x000; // both of theese prevent softlocks
    if (tbl->flags.byte == randTbl->flags.byte/*CONFIG_ENABLED(randomizeEnemies)*/) tbl->flags.formation = 0x000;
